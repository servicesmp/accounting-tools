...
class FacturXInvoice {
    ...
    buildHeaderTradeDelivery(tx) {
        ...
        if (shipTo?.address && this.profile !== types_1.FacturxProfile.MINIMUM) {
            const party = delivery.ele('ram:ShipToTradeParty');
            if (shipTo.name)
                party.ele('ram:Name').txt(shipTo.name);
            const addr = party.ele('ram:PostalTradeAddress');
            if (shipTo.address.postalCode)
                addr.ele('ram:PostcodeCode').txt(shipTo.address.postalCode);
            if (shipTo.address.street)
                addr.ele('ram:LineOne').txt(shipTo.address.street);
            if (shipTo.address.additionalStreet)
                addr.ele('ram:LineTwo').txt(shipTo.address.additionalStreet);
            if (shipTo.address.city)
                addr.ele('ram:CityName').txt(shipTo.address.city);
        ...
        dateTime.ele('udt:DateTimeString', { format: '102' })
            .txt((0, constants_1.formatDateFacturX)(this.header.deliveryDate ?? this.header.invoiceDate));
    }
    /**
    ...
    buildHeaderTradeSettlement(tx, summary) {
        ...
        for (const taxSummary of summary.taxSummaries) {
            ...
            if (this.header.vatDueDateTypeCode && this.profile !== types_1.FacturxProfile.MINIMUM) {
        ...
        if ((this.header.billingPeriodStart || this.header.billingPeriodEnd) &&
            this.profile !== types_1.FacturxProfile.MINIMUM) {
            const period = settlement.ele('ram:BillingSpecifiedPeriod');
            if (this.header.billingPeriodStart) {
                period.ele('ram:StartDateTime')
                    .ele('udt:DateTimeString', { format: '102' })
                    .txt((0, constants_1.formatDateFacturX)(this.header.billingPeriodStart));
            }
            if (this.header.billingPeriodEnd) {
                period.ele('ram:EndDateTime')
                    .ele('udt:DateTimeString', { format: '102' })
                    .txt((0, constants_1.formatDateFacturX)(this.header.billingPeriodEnd));
            }
        }
        ...
        if (preceding?.id) {
            ...
            if (preceding.issueDate) {
                ref.ele('ram:FormattedIssueDateTime')
                    ...
                    .txt((0, constants_1.formatDateFacturX)(preceding.issueDate));
            }
        }
    }
    /**
