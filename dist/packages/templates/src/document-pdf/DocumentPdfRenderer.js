...
exports.DocumentPdfRenderer = exports.PAGE_H = exports.PAGE_W = void 0;
exports.renderDocumentPdf = renderDocumentPdf;
/**
...
const pattern_1 = require("../document/pattern");
...
exports.PAGE_W = 794;
exports.PAGE_H = 1123;
const K = 595.28 / exports.PAGE_W; // px → pt
...
function assetPath(...p) {
    return path_1.default.join(__dirname, '../../assets', ...p);
}
function color(hex) {
    const h = hex.replace('#', '');
...
class DocumentPdfRenderer {
    constructor(view, options = {}) {
        ...
        this.L = SIDE;
        this.R = exports.PAGE_W - SIDE;
        this.v = view;
    }
    get frame() { return this.v.settings.layout.frame; }
    get primary() { return this.v.theme.primary; }
    get accent() { return this.v.theme.accent; }
    // ─── Primitives (coordonnées px, origine en haut à gauche) ────────────────
    font(bold) { return bold ? this.bold : this.regular; }
    prep(t, s) {
     ...
     */
    shape(txt, s) {
        const fk = this.font(s.bold).embedder?.font;
        ...
        run.glyphs.forEach((g, i) => {
            ...
            if (adv !== g.advanceWidth) {
                segments.push({ text: cur, x: curX });
                cur = '';
            }
        });
        if (cur)
    ...
    text(t, x, top, s, align = 'left', opacity) {
        const txt = this.prep(t, s);
        if (!txt)
            return;
        ...
        if (cs)
            this.page.pushOperators((0, pdf_lib_1.pushGraphicsState)(), (0, pdf_lib_1.setCharacterSpacing)(cs));
        for (const seg of shaped.segments) {
            this.page.drawText(seg.text, {
                x: (x + dx + seg.x) * K, y: (exports.PAGE_H - base) * K, size: s.size * K, font: this.font(s.bold),
                color: color(s.color ?? C.text), ...(opacity !== undefined ? { opacity } : {}),
            });
        }
        if (cs)
            this.page.pushOperators((0, pdf_lib_1.popGraphicsState)());
    }
    ...
    wrap(t, maxW, s) {
        ...
        for (const { t: w, sep } of parts) {
            ...
            if (cur && this.width(test, { ...s, upper: false }) > maxW + 0.01) {
                lines.push(cur);
                cur = w;
            }
            else
    ...
    para(t, x, top, maxW, s, align = 'left', draw = true) {
        const lines = this.wrap(t, maxW, s);
        const lh = lineHeight(s);
        if (draw)
            lines.forEach((l, i) => this.text(l, align === 'right' ? x + maxW : x, top + i * lh, { ...s, upper: false }, align));
    ...
    rect(x, y, w, h, o) {
        ...
        if (tl + tr + br + bl > 0) {
            ...
            const path = (i) => {
                const a = (r) => Math.max(0, r - i);
                const arc = (r, ex, ey) => (a(r) > 0 ? ` A${a(r)},${a(r)} 0 0 1 ${ex},${ey}` : ` L${ex},${ey}`);
            ...
            if (o.fill)
                this.page.drawSvgPath(path(0), { x: 0, y: exports.PAGE_H * K, scale: K, color: color(o.fill), ...(o.opacity !== undefined ? { opacity: o.opacity } : {}) });
            if (o.stroke)
                this.page.drawSvgPath(path((o.sw ?? 1) / 2), { x: 0, y: exports.PAGE_H * K, scale: K, borderColor: color(o.stroke), borderWidth: (o.sw ?? 1) * K });
            return;
        }
        if (o.fill)
            this.page.drawRectangle({ x: x * K, y: (exports.PAGE_H - y - h) * K, width: w * K, height: h * K, color: color(o.fill), ...(o.opacity !== undefined ? { opacity: o.opacity } : {}) });
        if (o.stroke) {
            const sw = o.sw ?? 1;
            this.page.drawRectangle({ x: (x + sw / 2) * K, y: (exports.PAGE_H - y - h + sw / 2) * K, width: (w - sw) * K, height: (h - sw) * K, borderColor: color(o.stroke), borderWidth: sw * K });
        }
    }
    ...
    image(img, x, y, w, h) {
        this.page.drawImage(img, { x: x * K, y: (exports.PAGE_H - y - h) * K, width: w * K, height: h * K });
    }
    ...
    logoIn(x, y, w, h) {
        ...
        const lw = this.logo.width * s;
        const lh = this.logo.height * s;
        this.image(this.logo, x + (w - lw) / 2, y + (h - lh) / 2, lw, lh);
    }
    get hasLogo() { return !!this.logo && this.v.settings.logo !== 'none'; }
    // ─── Point d'entrée ───────────────────────────────────────────────────────
    async render() {
        const v = this.v;
        this.doc = await pdf_lib_1.PDFDocument.create();
        this.doc.registerFontkit(fontkit_1.default);
        this.regular = await this.doc.embedFont(fs_1.default.readFileSync(assetPath('fonts', 'LiberationSans-Regular.ttf')), { subset: true });
        this.bold = await this.doc.embedFont(fs_1.default.readFileSync(assetPath('fonts', 'LiberationSans-Bold.ttf')), { subset: true });
        const embedsXml = !!this.options.facturXml;
        await (0, PDFA3Compliance_1.setupPDFA3Compliance)(this.doc, {
        ...
        if (b && b.length > 0 && v.settings.logo !== 'none') {
            try {
                this.logo = b[0] === 0x89 && b[1] === 0x50 ? await this.doc.embedPng(b) : await this.doc.embedJpg(b);
            }
            catch {
                this.logo = undefined;
            }
        ...
        if (v.poweredBy) {
            try {
                this.servicesLogo = await this.doc.embedPng(fs_1.default.readFileSync(assetPath('brand', 'services-logo.png')));
            }
            catch { /* facultatif */ }
        }
        if (v.payment?.qrData) {
            try {
                const png = await qrcode_1.default.toBuffer(v.payment.qrData, { type: 'png', width: 256, margin: 0, errorCorrectionLevel: 'M', color: { dark: '111111ff', light: 'ffffffff' } });
        ...
        if (this.options.facturXml) {
            ...
            await (0, AFRelationshipFix_1.attachFileWithAFRelationship)(this.doc, Buffer.from(this.options.facturXml, 'utf-8'), 'factur-x.xml', {
    ...
    // ─── Pages ────────────────────────────────────────────────────────────────
    newPage(first = false) {
        this.page = this.doc.addPage([exports.PAGE_W * K, exports.PAGE_H * K]);
        this.pages.push(this.page);
        const sb = this.frame === 'sidebar';
        this.L = sb ? SIDEBAR_MAIN_L : SIDE;
        this.R = exports.PAGE_W - SIDE;
        if (this.v.settings.pattern && !sb && first)
            this.pattern();
        if (sb)
            this.rect(0, 0, SIDEBAR_W, exports.PAGE_H, { fill: this.primary });
        if (this.v.watermark)
            this.watermark();
    }
    ...
    footerTop() {
        const v = this.v;
        if (this.frame === 'sidebar')
            return exports.PAGE_H - FOOT_BOTTOM - normalLH(8.5) - 4;
        const legalH = this.paraHeight(v.legal, this.legalWidth(), { size: 8.5, lh: 1.5 });
        return exports.PAGE_H - FOOT_BOTTOM - Math.max(legalH, this.footerGroupH()) - 12 - 1;
    }
    ...
    pattern() {
        ...
        const size = 420;
        const x = exports.PAGE_W + 120 - size;
        const y = -110;
        const scale = (size / pattern_1.PATTERN_4_VIEWBOX) * K;
        for (const d of pattern_1.PATTERN_4_PATHS) {
            this.page.drawSvgPath(d, { x: x * K, y: (exports.PAGE_H - y) * K, scale, color: color('#000000'), opacity: 0.07 });
        }
    }
    watermark() {
        // left:58%; top:54%; translate(-50%,-50%) rotate(-32deg); 700 110px; ls .06em; opacity .07
        const s = { size: 110, bold: true, ls: 0.06 };
        const txt = this.v.watermark;
        const w = this.width(txt, s);
        const h = normalLH(110);
        const cx = exports.PAGE_W * 0.58;
        const cy = exports.PAGE_H * 0.54;
        const a = (32 * Math.PI) / 180;
        // point bas-gauche de la ligne de base, avant rotation, relatif au centre (repère y vers le bas)
        const bx = -w / 2;
        const by = -h / 2 + baselineOffset(s);
        ...
        const cs = (s.ls ?? 0) * s.size * K;
        this.page.pushOperators((0, pdf_lib_1.pushGraphicsState)(), (0, pdf_lib_1.setCharacterSpacing)(cs));
        this.page.drawText(txt, { x: (cx + rx) * K, y: (exports.PAGE_H - (cy + ry)) * K, size: s.size * K, font: this.bold, color: color(this.primary), opacity: 0.07, rotate: (0, pdf_lib_1.degrees)(32) });
        this.page.pushOperators((0, pdf_lib_1.popGraphicsState)());
    }
    ...
    headerStandard() {
        ...
        const titleS = { size: 34, bold: true, ls: -0.01, color: C.ink };
        const subS = { size: 11, color: C.slate500 };
        ...
        const lab = { size: 11, color: C.slate500 };
    ...
    headerBand() {
        ...
        const nameS = { size: 19, bold: true, color: white };
        const subS = { size: 10, color: (0, color_1.blend)(white, this.primary, 0.78) };
        ...
        const titleS = { size: 28, bold: true, ls: 0.05, color: this.accent };
        const row1H = Math.max(leftH, normalLH(28));
        const cellsTop = top + row1H + 28;
        const cellValH = (sz) => 14 + normalLH(8) + 4 + normalLH(sz) + 14;
        const cellH = Math.max(cellValH(12), cellValH(14));
        const bandH = cellsTop + 1 + cellH;
        this.rect(0, 0, exports.PAGE_W, bandH, { fill: this.primary });
        if (v.settings.pattern)
            this.pattern();
        if (logo) {
        ...
        this.text(v.title, exports.PAGE_W - SIDE, top, titleS, 'right');
        // Cellules : N°, émission, échéance, Total TTC (cellule accent jusqu'au bord)
        this.hline(SIDE, exports.PAGE_W - SIDE, cellsTop, 1, (0, color_1.blend)(white, this.primary, 0.18));
        const cw = (exports.PAGE_W - 2 * SIDE) / 4;
        const cells = [
        ...
        cells.forEach((c, i) => {
            const x = SIDE + i * cw;
            this.text(c.l, x, cy + 14, { size: 8, ls: 0.1, upper: true, color: (0, color_1.blend)(white, this.primary, 0.65) });
            this.text(c.val, x, cy + 14 + normalLH(8) + 4, { size: 12, bold: true, color: white });
        });
        const tx4 = SIDE + 3 * cw;
        this.rect(tx4, cy, exports.PAGE_W - tx4, cellH, { fill: this.accent });
        this.text(v.labels.grandTotal, tx4 + 18, cy + 14, { size: 8, ls: 0.1, upper: true, color: (0, color_1.blend)(white, this.accent, 0.85) });
    ...
    headerSidebar() {
        ...
        const aS = { size: 10, lh: 1.6, color: (0, color_1.blend)(white, p, 0.8) };
        for (const l of addr)
            y += this.para(l, x, y, w, aS);
        // Bloc 2 : N°, dates
        y += 30;
        this.hline(x, x + w, y, 1, (0, color_1.blend)(white, p, 0.2));
        y += 1 + 22;
        const item = (label, val, acc = false) => {
            this.text(label, x, y, { size: 8, ls: 0.1, upper: true, color: (0, color_1.blend)(white, p, 0.65) });
            y += normalLH(8) + 3;
            this.text(val, x, y, { size: 12, bold: true, color: acc ? this.accent : white });
            y += normalLH(12);
        };
        item(v.labels.number, v.number);
        y += 12;
        item(v.labels.issue, v.issueDate);
        if (v.due) {
            y += 12;
            item(v.due.label, v.due.value, true);
        }
        // Bloc 3 : paiement + QR
        if (v.payment) {
            y += 30;
            this.hline(x, x + w, y, 1, (0, color_1.blend)(white, p, 0.2));
            y += 1 + 22;
            const pS = { size: 9.5, lh: 1.55, color: white };
            this.text(v.labels.paymentTerms, x, y, { size: 8, lh: 1.55, ls: 0.1, upper: true, color: (0, color_1.blend)(white, p, 0.65) });
            y += 8 * 1.55 + 8;
            if (v.payment.iban) {
                y += this.para(`${v.labels.iban} ${v.payment.iban}`, x, y, w, pS) + 8;
            }
            if (v.payment.bic) {
                y += this.para(`${v.labels.bic} ${v.payment.bic}`, x, y, w, pS) + 8;
            }
            if (this.qr) {
                y += 6;
        ...
        const lS = { size: 8, lh: 1.55, color: (0, color_1.blend)(white, p, 0.6) };
        const lh = this.paraHeight(v.legal, w, lS);
        this.para(v.legal, x, exports.PAGE_H - 34 - lh, w, lS);
        // Zone principale : titre 40 px + trait accent
        const tS = { size: 40, bold: true, lh: 1, ls: -0.02, color: p };
    ...
    headerHero() {
        ...
        let nx = this.L;
        if (this.hasLogo) {
            this.logoIn(this.L, top + (rowH - 30) / 2, 30, 30);
            nx += 40;
        }
    ...
    headerCentered() {
        ...
        y += normalLH(10) + 10 + 14;
        const pS = { size: 14, bold: true, ls: 0.14, color: this.accent };
    ...
    parties(y) {
        if (this.frame === 'sidebar')
            return this.partiesBuyer(y);
        if (this.frame === 'hero')
            return this.partiesRow(y);
        return this.partiesTwo(y);
    }
    get labelColor() { return this.frame === 'hero' ? C.gray : this.accent; }
    partiesTwo(y0) {
        const v = this.v;
        ...
        const lab = { size: 9, bold: true, ls: 0.1, upper: true, color: this.labelColor };
        const content = (p) => {
            const lines = [...p.street, p.cityLine].filter(Boolean);
            let h = normalLH(9) + 4 + 4 + normalLH(12);
            for (const l of lines)
                h += 4 + this.paraHeight(l, inner, { size: 11 });
            if (p.vat)
                h += 4 + 4 + normalLH(10);
            if (p.id && p !== v.seller)
                h += 4 + normalLH(10);
        ...
        const draw = (p, heading, x, buyer) => {
            const band = this.frame === 'band' && style !== 'plain' && buyer;
            const bg = band ? (0, color_1.mix)(this.accent, 0.10) : style === 'card' ? (0, color_1.mix)(this.accent, 0.08) : undefined;
            const bd = band ? (0, color_1.mix)(this.accent, 0.40) : style === 'framed' ? C.line : style === 'card' ? (0, color_1.mix)(this.accent, 0.35) : undefined;
            const r = style === 'card' ? 12 : 0;
            if (bg)
                this.rect(x, top, colW, h, { fill: bg, r });
            if (bd)
                this.rect(x, top, colW, h, { stroke: bd, sw: 1, r });
            ...
            y += normalLH(12);
            for (const l of [...p.street, p.cityLine].filter(Boolean)) {
                y += 4;
                y += this.para(l, cx, y, inner, { size: 11 });
            }
            if (p.vat) {
                y += 4 + 4;
                this.text(p.vat, cx, y, { size: 10, color: C.slate500 });
                y += normalLH(10);
            }
            if (p.id && buyer) {
                y += 4;
                this.text(p.id, cx, y, { size: 10, color: C.slate500 });
            }
        };
    ...
    partiesBuyer(y0) {
        ...
        if (v.buyer.vat) {
            y += 4;
            this.text(v.buyer.vat, this.L, y, { size: 10, color: C.slate500 });
            y += normalLH(10);
        }
        if (v.buyer.id) {
            y += 4;
            this.text(v.buyer.id, this.L, y, { size: 10, color: C.slate500 });
            y += normalLH(10);
        }
        return y;
    }
    partiesRow(y0) {
        ...
        const base = { size: 10.5, lh: 1.55 };
        const lab = { size: 8, lh: 1.55, ls: 0.12, upper: true, color: C.gray };
        const col = (x, heading, p) => {
            let y = top;
            this.text(heading, x, y, lab);
            y += 8 * 1.55 + 6;
            this.text(p.name, x, y, { ...base, bold: true });
            y += 10.5 * 1.55;
            y += this.para(p.oneLineNoCountry, x, y, colW, base);
            if (p.vat) {
                this.text(p.vat, x, y, { ...base, color: C.gray });
                y += 10.5 * 1.55;
            }
            return y - top;
        };
        ...
        let y = top;
        const x = this.L + 2 * (colW + gap);
        this.text(v.labels.issue, x, y, lab);
        y += 8 * 1.55 + 6;
        this.text(v.issueDate, x, y, { ...base, bold: true });
        y += 10.5 * 1.55;
        y += 8;
        this.text(v.labels.number, x, y, lab);
        y += 8 * 1.55 + 2;
        this.text(v.number, x, y, { ...base, bold: true });
        y += 10.5 * 1.55;
    ...
    refLine(y0) {
        ...
        if (!v.refLine)
            return y0;
        const top = y0 + 18;
        const s = { size: 10, color: C.slate700 };
    ...
    tableHeader(y) {
        ...
        const bordered = this.frame === 'centered';
        const hS = { size: 9, bold: true, ls: 0.06, upper: true, color: style === 'filled' ? C.white : this.primary };
        const h = 10 + normalLH(9) + 10;
        const bg = style === 'filled' ? this.primary : style === 'zebra' ? (0, color_1.mix)(this.primary, 0.12) : undefined;
        const x0 = this.L + (bordered ? 1 : 0);
        const x1 = this.R - (bordered ? 1 : 0);
        if (bordered)
            y += 1; // bordure haute du tableau arrondi
        const ir = bordered ? 11 : 0; // overflow:hidden + border-radius 12 − bordure 1
        if (bg)
            this.rect(x0, y, x1 - x0, h, { fill: bg, r: [ir, ir, 0, 0] });
        for (const g of geo)
            this.text(g.col.label, g.col.align === 'right' ? g.x + g.w : g.x, y + 10, hS, g.col.align);
        let bottom = y + h;
        if (style === 'lines') {
            this.hline(this.L, this.R, bottom, 1.5, this.primary);
            bottom += 1.5;
        }
        return bottom;
    }
    rowHeight(r, geo) {
        const s = { size: 10.5 };
        if (r.kind === 'group')
            return 22 + normalLH(10.5);
        let h = normalLH(10.5);
        for (const g of geo) {
            const t = r.cells[g.col.key] ?? '';
            h = Math.max(h, this.paraHeight(t, g.w, { ...s, bold: g.col.bold }) || normalLH(10.5));
            if (g.col.key === 'description' && r.details)
                h = Math.max(h, this.paraHeight(t, g.w, { ...s, bold: true }) + 2 + this.paraHeight(r.details, g.w, { size: 9.5, color: C.slate500 }));
        }
        return 22 + h;
    }
    table(y0) {
        ...
        let segTop = startY;
        const zebra = (0, color_1.mix)(this.primary, 0.05);
        let i = 0;
        for (const r of v.rows) {
            const h = this.rowHeight(r, geo) + (style === 'lines' ? 1 : 0);
            if (y + h > this.footerTop() - 8) {
                if (bordered)
                    this.rect(this.L, segTop, this.R - this.L, y - segTop, { stroke: C.rowRule, sw: 1, r: 12 });
                const ny = this.ensure(exports.PAGE_H, 0);
                segTop = ny;
            ...
            if (r.kind === 'group') {
                ...
                if (r.subtotal)
                    this.text(r.subtotal, geo[geo.length - 1].x + geo[geo.length - 1].w, y + 11, { size: 10.5, bold: true, color: this.primary }, 'right');
            }
            else {
                const last = r === v.rows[v.rows.length - 1];
                const ir = bordered && last ? 11 : 0;
                if (style === 'zebra' && i % 2 === 1)
                    this.rect(x0, y, x1 - x0, h, { fill: zebra, r: [0, 0, ir, ir] });
                for (const g of geo) {
                    const t = r.cells[g.col.key] ?? '';
                    const s = { size: 10.5, bold: g.col.bold, color: g.col.primary ? this.primary : C.text };
                    const ph = this.para(t, g.x, y + 11, g.w, s, g.col.align);
                    if (g.col.key === 'description' && r.details)
                        this.para(r.details, g.x, y + 11 + ph + 2, g.w, { size: 9.5, color: C.slate500 });
                }
                i++;
            }
            y += h;
            if (style === 'lines')
                this.hline(this.L, this.R, y - 1, 1, C.rowRule);
        }
        if (bordered)
            this.rect(this.L, segTop, this.R - this.L, y + 1 - segTop, { stroke: C.rowRule, sw: 1, r: 12 });
        return y + (bordered ? 1 : 0);
    }
    // ─── Bas de page : TVA, paiement, totaux ─────────────────────────────────
    bottom(y0) {
        if (this.frame === 'band')
            return this.bottomBand(y0);
        if (this.frame === 'centered')
            return this.bottomCard(y0);
    ...
    bottomStd(y0) {
        const v = this.v;
        const sb = this.frame === 'sidebar';
        const totW = sb ? 270 : 250;
        const gap = sb ? 16 : 26;
        const leftW = this.R - this.L - totW - gap;
        const lab = { size: 9, bold: true, ls: 0.1, upper: true, color: this.labelColor };
        const small = { size: 10, color: C.slate600 };
        // Hauteurs
        ...
        let y = top;
        if (v.taxes.length) {
            this.text(v.labels.taxBreakdown, this.L, y, lab);
            y += normalLH(9);
            for (const t of v.taxes) {
                y += 5;
                y += this.para(this.taxLine(t), this.L, y, leftW, small);
            }
            y += pay ? 14 : 0;
        }
        if (pay) {
            ...
            let ty = y;
            this.text(v.labels.paymentTerms, tx, ty, lab);
            ty += normalLH(9);
            for (const l of payLines) {
                ty += 3;
                ty += this.para(l, tx, ty, payTextW, small);
            }
            if (this.qr) {
                ty += 3;
                this.text(v.labels.scanToPay, tx, ty, { size: 10, bold: true, color: this.primary });
            }
        }
    ...
    grandBox(x, y, w) {
        ...
        if (style === 'block')
            this.rect(x, y, w, h, { fill: this.primary, r: 8 });
        if (style === 'tint')
            this.rect(x, y, w, h, { fill: (0, color_1.mix)(this.accent, 0.14), r: 8 });
        if (plain)
            this.hline(x, x + w, y, 2, this.primary);
    ...
    bottomBand(y0) {
        ...
        const w1 = (W * 1) / 3.4;
        const s = { size: 10, color: C.slate600 };
        const lab = { size: 9, bold: true, ls: 0.1, upper: true, color: this.primary };
        const inner = w1 - 32 - 1;
        ...
        const colH = (lines) => normalLH(9) + lines.reduce((a, l) => a + 5 + this.paraHeight(l, inner, s), 0);
        const white = C.white;
        const r = { size: 10.5, color: (0, color_1.blend)(white, this.primary, 0.8) };
        ...
        const col = (x, title, lines) => {
            let y = top + 1 + 14;
            this.text(title, x + 16, y, lab);
            y += normalLH(9);
            for (const l of lines) {
                y += 5;
                y += this.para(l, x + 16, y, inner, s);
            }
        };
        if (v.taxes.length)
            col(x1, v.labels.taxBreakdown, c1);
        if (v.payment)
            col(x2, v.labels.paymentTerms, c2);
        // Colonne totaux (fond principal)
        this.rect(x3, top + 1, this.R - 1 - x3, h, { fill: this.primary });
        let y = top + 1 + 14;
        const rx = x3 + 16;
        const rr = this.R - 1 - 16;
        const row = (l, val) => { this.text(l, rx, y, r); this.text(val, rr, y, r, 'right'); y += normalLH(10.5) + 6; };
        row(v.labels.subtotal, v.subtotal);
        row(v.labels.taxTotal, v.taxTotal);
        for (const c of v.extraCharges)
            row(c.label, c.value);
        ...
        y += 4;
        this.hline(rx, rr, y, 1, (0, color_1.blend)(white, this.primary, 0.25));
        const gy = y + 1 + 8;
        // alignement sur la ligne de base (align-items: baseline)
        const lb = { size: 10.5, bold: true, color: white };
        const gv = { size: 17, bold: true, color: this.accent };
    ...
    bottomCard(y0) {
        ...
        const s = { size: 10, color: C.slate600 };
        const lab = { size: 9, bold: true, ls: 0.1, upper: true, color: this.accent };
        const lines = [
        ...
        let y = top + 16;
        this.text(v.labels.paymentTerms, this.L + 18, y, lab);
        y += normalLH(9);
        for (const l of lines) {
            y += 6;
            y += this.para(l, this.L + 18, y, inner, s);
        }
        ...
        const white = C.white;
        const r = { size: 10.5, color: (0, color_1.blend)(white, this.primary, 0.85) };
        let ry = top + 18;
        ...
        for (const c of v.extraCharges) {
            ry += 6;
            this.text(c.label, x + 20, ry, r);
            this.text(c.value, x + w - 20, ry, r, 'right');
            ry += normalLH(10.5);
        }
        // dernière rangée collée en bas (justify-content: space-between)
        const lb = { size: 12, bold: true, color: white };
        const gv = { size: 26, bold: true, color: white };
    ...
    legalWidth() {
        ...
        return Math.min(500, exports.PAGE_W - 2 * SIDE - 20 - right);
    }
    ...
    footerRightWidth() {
        ...
        if (v.badge)
            w += 10 + this.width(v.badge, { size: 8, bold: true }) + 14 + 2;
        if (v.poweredBy)
            w += 10 + this.width(v.labels.poweredBy, { size: 8 }) + 6 + this.servicesLogoWidth() + 10 + 1;
        return w;
    }
    servicesLogoWidth() {
    ...
    footers() {
        ...
        this.pages.forEach((p, i) => {
            ...
            if (v.poweredBy) {
                ...
                const tS = { size: 8, color: C.slate400 };
                ...
                if (this.servicesLogo)
                    this.image(this.servicesLogo, bx + tw + 6, gTop + (groupH - 11) / 2, lw, 11);
                this.vline(bx + blockW - 1, gTop + (groupH - 11) / 2, gTop + (groupH + 11) / 2, 1, C.line);
            }
            if (!sb) {
                const lS = { size: 8.5, lh: 1.5, color: C.slate400 };
                const lw = this.legalWidth();
                const lh = this.paraHeight(v.legal, lw, lS);
                const h = Math.max(lh, groupH);
                this.hline(SIDE, exports.PAGE_W - SIDE, bottom - h - 12 - 1, 1, C.line);
                this.para(v.legal, SIDE, bottom - lh, lw, lS);
            }
        });
    }
}
exports.DocumentPdfRenderer = DocumentPdfRenderer;
async function renderDocumentPdf(view, options = {}) {
    return new DocumentPdfRenderer(view, options).render();
}
//# sourceMappingURL=DocumentPdfRenderer.js.map