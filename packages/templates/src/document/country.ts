...
function nameIndex(): Map<string, string> {
  ...
  for (const locale of ['fr', 'en', 'de', 'es', 'it', 'pt']) {
    let dn: any;
    try { dn = DisplayNames ? new DisplayNames([locale], { type: 'region' }) : null; } catch { dn = null; }
    if (!dn) continue;
    for (const code of ISO_ALPHA2) {
      const name = dn.of(code);
      if (name && name !== code) byName.set(fold(name), code);
    }
  }
  for (const [k, v] of Object.entries(ALIASES)) byName.set(fold(k), v);
  return byName;
}
 ...
 */
export function toIsoCountryCode(input: unknown): string | null {
...
export function toIsoCountryCodeOr(input: unknown, fallback: string): string {
  return toIsoCountryCode(input) ?? fallback;
}

/** Nom lisible d'un pays dans la langue du document (« FR » → « France »). */
export function countryDisplayName(code: string | null | undefined, language: string = 'fr'): string {
