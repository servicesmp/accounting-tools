 ...
 */
import type { DocumentView, ViewParty, ViewRow } from './types';
import { blend, mix, normalizeHex } from './color';
import { PATTERN_4_PATHS, PATTERN_4_VIEWBOX } from './pattern';
...
export function safeImageUrl(url: unknown): string | null {
  ...
  if (/^data:image\/(png|jpe?g|gif|webp|svg\+xml);base64,[a-z0-9+/=\s]+$/i.test(u)) return u.replace(/\s+/g, '');
  return null;
}
...
function fontStack(f: string | undefined): string {
...
export function renderDocumentHtml(view: DocumentView, options: RenderDocumentHtmlOptions = {}): string {
  const v = view;
  const s = v.settings;
  const L = v.labels;
  ...
  const accentTint = color(v.theme.accentTint, mix(accent, 0.14));
  ...
  const white = C.white;
  const F = (w: number, size: number, lineHeight = '') => `font:${w} ${size}px${lineHeight} ${font}`;
  const lab = (c: string) => `${F(700, 9)};letter-spacing:.1em;text-transform:uppercase;color:${c}`;
...
<div style="display:grid;grid-template-columns:auto auto;gap:6px 18px;font-size:11px">${meta}</div>
  ...
  const headerBand = () => {
    const cell = (label: string, value: string) =>
      `<div style="padding:14px 0;display:flex;flex-direction:column;gap:4px"><span style="font-size:8px;color:${blend(white, primary, 0.65)};text-transform:uppercase;letter-spacing:.1em">${e(label)}</span><span style="${F(700, 12)}">${e(value)}</span></div>`;
...
<div style="${logoWrap}">${logoBox(50, 40, white, '12px')}<div style="display:flex;flex-direction:column;gap:4px"><span style="${F(700, 19)}">${e(v.seller.name)}</span><span style="${F(400, 10)};color:${blend(white, primary, 0.78)}">${e([v.seller.oneLineNoCountry, v.seller.id].filter(Boolean).join(' · '))}</span></div></div>
<span style="${F(700, 28)};letter-spacing:.05em;color:${accent}">${e(v.title)}</span></div>
<div style="position:relative;margin-top:28px;display:grid;grid-template-columns:repeat(4,1fr);border-top:1px solid ${blend(white, primary, 0.18)}">
${cell(L.number, v.number)}${cell(L.issue, v.issueDate)}${v.due ? cell(v.due.label, v.due.value) : '<div></div>'}
<div style="padding:14px 0;display:flex;flex-direction:column;gap:4px;background:${accent};margin-right:-56px;padding-left:18px"><span style="font-size:8px;color:${blend(white, accent, 0.85)};text-transform:uppercase;letter-spacing:.1em">${e(L.grandTotal)}</span><span style="${F(700, 14)}">${e(v.grandTotal)}</span></div>
</div></div>`;
  };

  const headerSidebarTitle = () =>
    `<div style="padding:52px 0 0;display:flex;flex-direction:column;gap:6px"><span style="${F(700, 40, '/1')};letter-spacing:-.02em;color:${primary}">${e(v.title)}</span><span style="width:48px;height:4px;background:${accent};margin-top:10px"></span></div>`;
...
<span style="${F(400, 11)};color:${C.slate600}">${e(meta)}</span>
  ...
  const partyBg = partyStyle === 'card' ? mix(accent, 0.08) : 'transparent';
  const partyBorder = partyStyle === 'framed' ? `1px solid ${C.line}` : partyStyle === 'card' ? `1px solid ${mix(accent, 0.35)}` : 'none';
  const bandBuyer = frame === 'band' && partyStyle !== 'plain';

  const partyBox = (p: ViewParty, heading: string, buyer: boolean) => {
    const bg = buyer && bandBuyer ? mix(accent, 0.10) : partyBg;
    const border = buyer && bandBuyer ? `1px solid ${mix(accent, 0.40)}` : partyBorder;
  ...
  const partiesTwo = () =>
    `<div style="padding-top:28px;display:grid;grid-template-columns:1fr 1fr;gap:18px">${partyBox(v.seller, L.seller, false)}${partyBox(v.buyer, L.buyer, true)}</div>`;

  const partiesBuyer = () =>
    `<div style="padding-top:34px;display:flex;flex-direction:column;gap:4px;font-size:11px"><span style="${lab(labelC)};margin-bottom:6px">${e(L.buyer)}</span><span style="font-weight:700;font-size:16px;color:${C.ink}">${e(v.buyer.name)}</span><span>${e(v.buyer.oneLine)}</span>${v.buyer.vat ? `<span style="color:${C.slate500};font-size:10px">${e(v.buyer.vat)}</span>` : ''}${v.buyer.id ? `<span style="color:${C.slate500};font-size:10px">${e(v.buyer.id)}</span>` : ''}</div>`;

  const partiesRow = () => {
    const small = `font-size:8px;letter-spacing:.12em;text-transform:uppercase;color:${C.gray}`;
    const col = (heading: string, p: ViewParty) =>
      `<div style="display:flex;flex-direction:column"><span style="${small};margin-bottom:6px">${e(heading)}</span><b>${e(p.name)}</b><span>${e(p.oneLineNoCountry)}</span>${p.vat ? `<span style="color:${C.gray}">${e(p.vat)}</span>` : ''}</div>`;
...
<div style="display:flex;flex-direction:column"><span style="${small};margin-bottom:6px">${e(L.issue)}</span><b>${e(v.issueDate)}</b><span style="${small};margin:8px 0 2px">${e(L.number)}</span><b>${e(v.number)}</b></div>
  ...
  const table = () => {
    ...
    const thBg = style === 'filled' ? primary : style === 'zebra' ? mix(primary, 0.12) : 'transparent';
...
${v.rows.map(row).join('\n')}
  ...
  const taxLine = (t: { rate: string; base: string; amount: string }) => `${t.rate} · ${L.taxBase} ${t.base} · ${L.taxAmount} ${t.amount}`;

  const grandBox = () => {
    const t = s.layout.totals;
    const pad = t === 'plain' ? '10px 0 0' : '12px 14px';
    const bg = t === 'block' ? primary : t === 'tint' ? mix(accent, 0.14) : 'transparent';
...
<div style="display:flex;flex-direction:column;gap:7px;font-size:11px">${row(L.subtotal, v.subtotal)}${row(L.taxTotal, v.taxTotal)}${v.extraCharges.map((c) => row(c.label, c.value)).join('')}${grandBox()}</div>
  ...
  const bottomBand = () => {
    const col = (title: string, lines: string[]) =>
      `<div style="padding:14px 16px;border-right:1px solid ${C.line};display:flex;flex-direction:column;gap:5px;font-size:10px;color:${C.slate600}">${title ? `<span style="${lab(primary)}">${e(title)}</span>` : ''}${lines.map((l) => `<span>${e(l)}</span>`).join('')}</div>`;
    ...
    const r = `display:flex;justify-content:space-between;gap:12px;color:${blend(white, primary, 0.8)}`;
...
<div style="padding:14px 16px;background:${primary};color:#fff;display:flex;flex-direction:column;gap:6px;font-size:10.5px">${rows}<div style="display:flex;justify-content:space-between;align-items:baseline;margin-top:4px;padding-top:8px;border-top:1px solid ${blend(white, primary, 0.25)}"><span style="font-weight:700">${e(L.grandTotal)}</span><span style="${F(700, 17)};color:${accent};white-space:nowrap">${e(v.grandTotal)}</span></div></div>
</div>`;
  };
  ...
  const bottomCard = () => {
    ...
    const extra = v.extraCharges.map((c) => `<div style="display:flex;justify-content:space-between;font-size:10.5px;color:${blend(white, primary, 0.85)}"><span>${e(c.label)}</span><span>${e(c.value)}</span></div>`).join('');
...
<div style="padding:18px 20px;border-radius:14px;background:${primary};color:#fff;display:flex;flex-direction:column;justify-content:space-between;gap:10px"><div style="display:flex;flex-direction:column;gap:6px"><div style="display:flex;justify-content:space-between;gap:12px;font-size:10.5px;color:${blend(white, primary, 0.85)}"><span>${e(`${L.subtotal} ${v.subtotal}`)}</span><span>${e(`${L.taxTotal} ${v.taxTotal}`)}</span></div>${extra}</div><div style="display:flex;justify-content:space-between;align-items:baseline"><span style="font-weight:700;font-size:12px">${e(L.grandTotal)}</span><span style="${F(700, 26)};white-space:nowrap">${e(v.grandTotal)}</span></div></div>
</div>`;
  };
  ...
  // ── Pied ──────────────────────────────────────────────────────────────────
  const powered = (extra: string) => (v.poweredBy
...
<div style="display:flex;align-items:center;gap:10px;flex:none">${powered('')}${badge()}${page}</div>
  ...
  const sidebar = () => {
    const faint = blend(white, primary, 0.65);
...
<div style="display:flex;flex-direction:column;gap:12px;padding-top:22px;border-top:1px solid ${blend(white, primary, 0.2)};font-size:10px">${item(L.number, v.number)}${item(L.issue, v.issueDate)}${v.due ? item(v.due.label, v.due.value, true) : ''}</div>
${pay}
<span style="margin-top:auto;${F(400, 8, '/1.55')};color:${blend(white, primary, 0.6)}">${e(v.legal)}</span>
</div>`;
  };

  function pattern(): string {
    const paths = PATTERN_4_PATHS.map((d) => `<path d="${e(d)}"/>`).join('');
    return `<svg aria-hidden="true" viewBox="0 0 ${PATTERN_4_VIEWBOX} ${PATTERN_4_VIEWBOX}" style="position:absolute;right:-120px;top:-110px;width:420px;height:420px;opacity:.07;pointer-events:none" fill="#000">${paths}</svg>`;
  }
